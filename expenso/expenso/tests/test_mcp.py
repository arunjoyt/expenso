...
from expenso.mcp import _json_safe, build_mcp_read_tool_schema, get_analytics, get_budgets, get_expenses, get_income
...
class TestMcpReadTools(FrappeTestCase):
	...
	def test_call_with_token_missing_read_scope_is_rejected(self):
		...
		frappe.set_user(self.member)
		with patch.object(frappe, "get_request_header", return_value=f"Bearer {write_only_token.access_token}"):
			self.assertRaises(frappe.PermissionError, get_expenses, month=6, year=2025)
