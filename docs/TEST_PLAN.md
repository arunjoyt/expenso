...
| # | Test | Assertion |
|---|------|-----------|
| F139 | Attach-photo control | an icon in the compose bar opens a two-option menu ("Take Photo" / "Photo Library") that sets `capture="environment"` or clears it on the hidden `accept="image/*"` input before clicking it, so mobile reliably offers both a camera and a library option instead of depending on the browser's default chooser; a picked file renders as a removable thumbnail chip above the input before sending |
