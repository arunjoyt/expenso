...
describe("BudgetSheet", () => {
	...
	it("shows the Category name in the sheet title", () => {
		const wrapper = mount(BudgetSheet, { props: { category: groceries } });
		expect(wrapper.text()).toContain("Groceries");
	});

	// F44
	it("pre-fills the amount input with the existing Budget", () => {
		const wrapper = mount(BudgetSheet, { props: { category: groceriesWithBudget } });
	...
	// F43
	it("saves the Budget on submit and closes", async () => {
		const wrapper = mount(BudgetSheet, { props: { category: groceries } });
		await amountInput(wrapper).setValue("240");
		await wrapper.find("form").trigger("submit.prevent");
		await flushPromises();

		expect(setBudget).toHaveBeenCalledWith("CAT-1", 240);
	...
	// F58
	it("removes the Budget when Remove Budget is clicked", async () => {
		const wrapper = mount(BudgetSheet, { props: { category: groceriesWithBudget } });
