...
| P6-S6 | `[FE]` (+ tiny `[A]`) | #70 (reused) | Chat surface as a 5th bottom-nav tab **"Assistant"** (replaces the 2026-09-09 floating bubble — ADR 0008's 2026-09-10 P6-S6 update); `pages/Assistant.vue` full-screen screen, transient step log + streamed prose, history from the thread, "Clear chat"; FAB extracted from `Feed.vue` into a global `Fab.vue` (all screens except Assistant) + shared `useEntrySheet.js`; `useAssistant.js` = read-only token mint + `fetch()`/SSE parsing + dormant unread badge. Companion `[A]` change in `expenso-assistant`: `CORSMiddleware` for the Frappe origin. New `expenso_assistant_url` in `site_config` → boot context. |
| P6-S7 | `[A]`+`[FE]` | #95 | Agent writes + confirm-card flow: `route()` sends write tool-calls to a `propose` node that raises `interrupt({actions})` directly (no elicitation bridge); batched per turn, before→after diff from the tool history, checkbox-per-action deselect/cancel; `/resume` re-derives the actions and executes the `{selected}` subset with `if_modified_since` (per-action conflict → re-read + re-propose); `entry_method=assistant`, no `is_external_write`; `max_proposed_writes_per_turn` batch cap **replaces** the daily write cap (ADR 0008's 2026-09-10 P6-S7 update); `resume_turn` re-mints + binds entry method. Frontend `ConfirmCard.vue` + `useAssistant.resume()` + `needs_confirmation` parsing; token minted `write=True`. FastMCP adapter keeps its own SEP-2322 confirm for external connectors. |
...
| P7-S2 | `[F]`+`[A]`+`[FE]` | #97 | Proactive Insights: `hooks.py` `scheduler_events` (monthly 1st, weekly — **off-hours slot** so a batch graph can't stall live chat streams) → per-Member read-token → fire-and-forget `POST /run/proactive` (`202`, graph runs as a background task) → read-only graph (`READ_TOOLS` only — never a proposal, binding-level guarantee) → Insight messages only, tagged for the transcript. `run_budget_drift` gates on a deterministic pre-check (`compute_budget_status` via `get_analytics`) and **re-warns every week with no dedup marker** (ADR 0008's 2026-09-11 P7-S2 update). Frontend unread badge: `BottomNav.vue` fetches history once on app-shell mount, no polling. Also: the **saved Langfuse dashboard views** (cost by day / by `user_id`, latency p50/p95, count by `feature` tag) — created in the running Langfuse over the SSH tunnel, documented as a `docs/DEPLOYMENT.md` runbook section (deferred here from P6-S5) |

---

## Phase 8 — Post-launch enhancements

**Goal:** Small UX improvements to the shipped Phases 1–7 surface, outside the original phased scope.

| Streak | Repo | Issue | Title |
|--------|------|-------|-------|
| P8-S1 | `[FE]` | #105 | Analytics: tap a Category with spend to expand it inline and see the matching Expenses for that month; tapping an expanded Expense opens the existing Edit sheet. Client-side filter against the already-fetched month's Expense list (`category_name` label match) — no backend changes |
